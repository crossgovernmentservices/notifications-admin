...
def ags_register():
    ...
    if not feature_switch_active():
        current_app.logger.debug('Feature switch not active')
        return redirect(url_for('main.register'))

    current_app.logger.debug('Feature switch active')

    if current_user.is_authenticated:
        current_app.logger.debug('User already logged in')
        return redirect_to_services()

    auth_data = request.environ.get('auth_data')
    current_app.logger.debug('Auth data: {}'.format(auth_data))

    if auth_data is None:
        current_app.logger.warning('No auth data in wsgi environ')
    ...
    if 'email' in auth_data['userinfo']:
        ...
        current_app.logger.warning('No email in userinfo')

    if user:
        current_app.logger.debug('Login user')
        login_user(user, remember=True)
        return redirect_to_services()

    if registration_form_submitted():
        current_app.logger.debug('Register user')
        user = register_user()
        ...
        current_app.logger.warning('User not logged in')
    ...
    try:
        ...
        if get_user(user.email_address):
            current_app.logger.error(
                'User already registered: {}'.format(user.email_address))
            raise UserExists()
...
def registration_form():
    ...
    current_app.logger.debug('Prepopulate registration form with: {}'.format(
        auth_data))
    ...
    if request.method.upper() == 'POST':
        ...
        if not form.validate():
            current_app.logger.warning(
                'Register form validation failed: {}'.format(form.errors))
            return False
